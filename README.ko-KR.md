...
FFmpeg 섹션에서 시스템 `PATH`의 실행 파일이나 사용자 지정 위치의 실행 파일을
선택할 수 있습니다.

## 프레임 선택

다음 세 가지 추출 방식을 지원합니다.

- 일정한 프레임 간격으로 추출
- 0부터 시작하는 특정 프레임 번호로 추출
- 특정 PTS(프레젠테이션 타임스탬프) 값으로 추출

추출 중에는 진행 상태와 진행률이 표시됩니다. 추출을 취소할 수 있으며, 추출이
진행 중인 상태에서 애플리케이션을 닫으면 확인 메시지가 표시됩니다.
